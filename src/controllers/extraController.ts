...
export const getJourneySteps = async (req: Request, res: Response) => {
  try {
...
export const createJourneyStep = async (req: Request, res: Response) => {
...
export const updateJourneyStep = async (req: Request, res: Response) => {
  const { number, title, subtitle, description, order } = req.body;
  try {
    const journeyStepId = String(req.params.id);
    const existing = await prisma.journeyStep.findUnique({ where: { id: journeyStepId } });
    if (!existing) return res.status(404).json({ message: "Journey step not found." });
...
export const deleteJourneyStep = async (req: Request, res: Response) => {
  try {
    const journeyStepId = String(req.params.id);
    const existing = await prisma.journeyStep.findUnique({ where: { id: journeyStepId } });
    if (!existing) return res.status(404).json({ message: "Journey step not found." });
